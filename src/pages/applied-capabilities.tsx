          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {Object.values(capabilityDetails).map((capability) => (
                <div
                  key={capability.id}
                  className="group relative p-6 rounded-xl border-2 border-border bg-white text-left transition-all duration-300 ease-in-out shadow-md hover:shadow-2xl hover:border-accent hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
                  aria-label={`Signal Intelligence capability ${capability.name}`}
                >
                  {/* Index number badge */}
                  <div className="absolute top-4 left-4 w-8 h-8 rounded-full bg-[#1e3a8a] flex items-center justify-center shadow-sm">
                    <span className="text-sm font-bold text-white">{capability.index}</span>
                  </div>

                  {/* Capability name */}
                  <div className="mt-8 mb-3 pl-10">
                  ...
                  {/* Full definition text visible */}
                  <p className="text-sm text-muted-foreground mb-4 pl-10">{capability.definition}</p>

                  <div className="text-2xl font-bold text-primary pl-10">{capability.exampleScore}</div>
                </div>
              ))}
            </div>
          </div>
